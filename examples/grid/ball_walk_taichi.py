...
field). Each is ingested by FOUR separate TaichiKernelBuilders, one per grid
config, differing only in which grid Bag gets bound under the name `grid`.
Nothing in either template body changes; make_grid's own block substitution
(see grid/_closure_blocks.py) is what makes `grid.neighbour(i, k)` and
`grid.neighbour_and_distance(i, k)` mean something different per panel.
...
def spread_template(d_out: ti.template(), d_in: ti.template()):
...
for panel_idx, (title, boundary, nodata) in enumerate(PANEL_CONFIGS):
    grid_bag = make_grid(
        "taichi", pool, NX, NY, DX, topology="D8", boundary=boundary, nodata=nodata
    )
    if nodata:
        grid_bag.nodata_mask.set(island_mask_flat)

    centre_p = TaichiParameter("CENTRE", dtype=ti.i32, mode="scalar", value=START_IDX, pool=pool)
    panel_seed = (SEED_VALUE + panel_idx * SEED_STRIDE) & 0x7FFFFFFF
    seed_p = TaichiParameter("SEED", dtype=ti.u32, mode="scalar", value=panel_seed, pool=pool)
    ...
    spread_kernel = TaichiKernelBuilder().bind("grid", grid_bag).ingest(spread_template).compile()

    d0 = pool.get_data(ti.f32, (NN,))
    d1 = pool.get_data(ti.f32, (NN,))
...
for panel, ax in zip(panels, axes.ravel()):
    ...
    panel["ax"] = ax
    ims.append(im)
fig.suptitle("Ball walk (Taichi) - one kernel template, four grids")
fig.tight_layout()
fig.show()
