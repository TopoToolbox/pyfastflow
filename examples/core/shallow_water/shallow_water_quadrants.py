...
core (Parameter/Helper/Kernel/Pool + Bags), Quadrants backend.

Same model as shallow_water_taichi.py: the Kass & Miller (1990) stable
shallow-water update on an Arakawa-C staggered grid. Cell-centered water
column height h, x-velocity u on vertical faces, y-velocity v on horizontal
faces:
...
rest_depth_p = QuadrantsParameter("REST_DEPTH", dtype=qd.f32, mode="const", value=REST_DEPTH_VAL, pool=pool, solo=True)
drop_r_p = QuadrantsParameter("DROP_R", dtype=qd.i32, mode="const", value=DROP_R_VAL, pool=pool, solo=True)
...
g_p = QuadrantsParameter("g", dtype=qd.f32, mode="scalar", value=G_VAL, pool=pool)
...
damp_p = QuadrantsParameter("damp", dtype=qd.f32, mode="const", value=DAMP_VAL, pool=pool)
phys = Bag({"g": g_p, "dx": dx_p, "dt": dt_p, "damp": damp_p})

# drop Bag: splash site + amplitude, host-set each time a stone falls.
drop_cx_p = QuadrantsParameter("cx", dtype=qd.i32, mode="scalar", value=GRID_N // 2, pool=pool)
drop_cy_p = QuadrantsParameter("cy", dtype=qd.i32, mode="scalar", value=GRID_N // 2, pool=pool)
drop_amp_p = QuadrantsParameter("amp", dtype=qd.f32, mode="scalar", value=0.0, pool=pool)
...
face_depth_fn = QuadrantsHelperBuilder().ingest(face_depth)
...
def init_height_template(h: qd.Tensor):
    for i, j in h:
        h[i, j] = REST_DEPTH


init_height_kernel = QuadrantsKernelBuilder().bind("REST_DEPTH", rest_depth_p).ingest(init_height_template).compile()


def apply_drop_template(h: qd.Tensor):
    for i, j in h:
        dxr = i - drop.cx.get(0)
        dyr = j - drop.cy.get(0)
...
def update_velocity_template(u: qd.Tensor, v: qd.Tensor, h: qd.Tensor):
    for i, j in h:
...
update_velocity_kernel = QuadrantsKernelBuilder().bind("phys", phys).ingest(update_velocity_template).compile()


def update_height_template(h_out: qd.Tensor, h_in: qd.Tensor, u: qd.Tensor, v: qd.Tensor):
    for i, j in h_in:
...
# ---------------------------------------------------------------------------
h0 = pool.get_data(qd.f32, (GRID_N, GRID_N))
h1 = pool.get_data(qd.f32, (GRID_N, GRID_N))
u = pool.get_data(qd.f32, (GRID_N, GRID_N))
v = pool.get_data(qd.f32, (GRID_N, GRID_N))
...
ax.set_title("Shallow-water waves in a tank (Quadrants backend)")
time_text = ax.text(
