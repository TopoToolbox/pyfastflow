"""
Same model and setup as heat_diffusion_taichi.py, with the per-substep
ping-pong expressed as a Routine instead of a hand-written python loop.

heat_diffusion_taichi.py alternates `diffuse_kernel(T1, T0)`,
...
iterations that one swap-pair covers are unrolled into one routine with a
repeat block, add_swap standing in for the python-level `T0, T1 = T1, T0`:
...
exactly as in heat_diffusion_taichi.py; apply_source_builder is also compiled
...
each builder already binds, so nothing about diffuse_template or
...
ax.set_title("Heat diffusion in a floor plan (Taichi backend, Routine)")
time_text = ax.text(
