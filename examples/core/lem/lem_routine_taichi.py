...
seed_a_p = TaichiParameter("SEED_A", dtype=ti.f32, mode="const", value=12.9898, pool=pool)
seed_b_p = TaichiParameter("SEED_B", dtype=ti.f32, mode="const", value=78.233, pool=pool)
...
def init_topo_template(z: ti.template()):
    ...
    for i, j in z:
        x = ti.cast(i, ti.f32) * SEED_A.get(0) + ti.cast(j, ti.f32) * SEED_B.get(0)
