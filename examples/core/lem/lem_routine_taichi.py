...
seed_a_p = TaichiParameter("SEED_A", dtype=ti.f32, mode="const", value=12.9898, pool=pool, solo=True)
seed_b_p = TaichiParameter("SEED_B", dtype=ti.f32, mode="const", value=78.233, pool=pool, solo=True)
...
d_p = TaichiParameter("D", dtype=ti.f32, mode="scalar", value=D_VAL, pool=pool)
sea_p = TaichiParameter("SEA_LEVEL", dtype=ti.f32, mode="scalar", value=0.0, pool=pool)

# field: one value per node, filled from the host below
uplift_p = TaichiParameter(
    "UPLIFT", dtype=ti.f32, mode="field", value=np.zeros(GRID_N * GRID_N), pool=pool, n_flat=GRID_N * GRID_N
)
...
clampi_fn = TaichiHelperBuilder().bind("N", n_p).ingest(clampi)
...
laplacian_fn = TaichiHelperBuilder().bind("clampi", clampi_fn).bind("grid", grid).ingest(laplacian)
...
uplift_at_fn = TaichiHelperBuilder().bind("UPLIFT", uplift_p).bind("N", n_p).ingest(uplift_at)
...
def init_topo_template(z: ti.template()):
    # bind_bag put SEED_A / SEED_B in flat, so they read as bare names here
    for i, j in z:
        x = ti.cast(i, ti.f32) * SEED_A + ti.cast(j, ti.f32) * SEED_B
        s = ti.sin(x) * 43758.5453
        z[i, j] = (s - ti.floor(s)) * 2.0


init_topo_kernel = TaichiKernelBuilder().bind_bag(noise_seeds).ingest(init_topo_template).compile()
...
def diffuse_template(z_out: ti.template(), z_in: ti.template()):
    for i, j in z_in:
        z_out[i, j] = z_in[i, j] + hill.dt * hill.d.get(0) * hill.lap(z_in, i, j)


diffuse_builder = TaichiKernelBuilder().bind("hill", hill).ingest(diffuse_template)


def uplift_template(z: ti.template()):
    for i, j in z:
        z[i, j] += up(i, j) * DT
...
# ---------------------------------------------------------------------------
z0 = pool.get_data(ti.f32, (GRID_N, GRID_N))
z1 = pool.get_data(ti.f32, (GRID_N, GRID_N))
...
ax.set_title("Hillslope LEM (Taichi backend, Routine)")
time_text = ax.text(
