...
drainage area, on Quadrants.

The reconstruction alternative to depressions_quadrants.py's carve/label/
saddle loop, on the new builder/frozen/bound stack (pyfastflow/experimental/
core/context/builder.py, frozen.py, bound.py): make_fill_reconstruct/
...
filled = pool.get_data(qd.f32, (n_flat,))
parent = pool.get_data(qd.i32, (n_flat,))
frontier = pool.get_data(qd.i32, (2 * n_flat,))
counters = pool.get_data(qd.i32, (MAX_PASSES + 2,))
queued_gen = pool.get_data(qd.i32, (n_flat,))
...
pass_p = QuadrantsParameter("PASS", dtype=qd.i32, mode="scalar", value=0, pool=pool)
active_p = QuadrantsParameter("ACTIVE", dtype=qd.i32, mode="scalar", value=0, pool=pool)
deps = make_fill_reconstruct("quadrants", grid_group, nx=N, ny=N)
solver = make_fill_reconstruct_solver(
    "quadrants", deps, grid_params,
