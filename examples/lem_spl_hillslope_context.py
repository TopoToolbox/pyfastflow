...
lemctx = LEMContext(
    ...
    K_bedrock=K_BEDROCK,
    kappa_bedrock_mode="const",
    kappa_bedrock=KAPPA_BEDROCK,
    m_exp_mode="const",
    m_exp=M_EXP,
    n_exp_mode="const",
    n_exp=N_EXP,
...
shade_np = visuctx.generate_hillshade(z, output_layout="2d")


fig, ax = plt.subplots(figsize=(10, 7), constrained_layout=True)
terrain_im = ax.imshow(terrain_np, cmap="terrain")
shade_im = ax.imshow(shade_np, cmap="gray", alpha=0.65, vmin=0.0, vmax=1.0)
ax.set_title("LEMContext SPL + Hillslope")
ax.set_axis_off()
fig.show()


for outer in range(N_OUTER):
    lem_runtime.run_spl_hillslope(
        lemctx,
        z,
    ...
    shade_im.set_data((shade_np-shade_np.min())/(shade_np.max()-shade_np.min()))
    ax.set_title(
        f"LEMContext SPL + Hillslope | outer={outer + 1}/{N_OUTER} | inner={N_INNER} | hs={HILLSLOPE_SUBSTEPS}"
    )
