...
# ---------------------------------------------------------------------------
# reconstruction fill (make_fill_reconstruct) - grayscale morphological
# reconstruction against elevation, ported from
# experimental/LM/fill_reconstruct_optimised.py's Round 4/5. See that file's
# module docstring for the full derivation of the direct-elevation-space
# formulation (filled[i] = max(z[i], min over neighbours filled), decreasing
# from a +inf interior sentinel to a fixed point) and of every optimisation
# below (queued_gen dedup instead of a per-pass reset, gated pushes instead of
# unconditional ones, four directional sweeps seeding the frontier, counters[]
# replacing two scalars reset every pass).
#
...
# (build_ping_pong_helpers/build_rake_compress above) - required, not built
# here, since this factory takes no pool.
#
...
def build_fill_reconstruct_relax(KernelCls, *, grid, pass_p, n_flat: int):
    """
    ...
    `pass_p` (the caller's `P` Parameter) is read only here, via
    `$P.get(0)$` - bumping it between passes is the caller's job (a
    host_step in the Sequence the solver builds), the same division of
    labour `iteration_p` has for rake_compress.

    Author: B.G (07/2026)
    """
    t = f"pfr{new_uid()}"
    return KernelCls().bind("grid", grid).bind("P", pass_p).ingest(
        f"""
